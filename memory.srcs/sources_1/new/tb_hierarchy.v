...
reg  [(4*b)-1:0] 	valid_bytes 		 [1:2];

reg  [(32*b)-1:0]	write_data 			 [1:2];
wire [(32*b)-1:0]	read_data 			 [1:2];
	...
	if(rst) begin
		...
		case (state)
			...
			write_st: begin
				case (sub_state)
					init: begin
						write_data[1] [(((address % (4*b))+1)*8)-1 -: 8] <= valToWrite[1];
						$display("write_data [%0d -: 8] <= %0d", ((address+1)*8)-1, valToWrite[1]);
						op[1] 			     <= `write_op;
						mem_operation[1]    <= 1'b1;
